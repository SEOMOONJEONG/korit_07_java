...
public class StrMap {
    public static void main(String[] args) {
        ...
        // 입력한 순서대로 출력되지 않습니다.
        System.out.println(strMap1); // 출력 : {키 = 값, 키 = 값, 키 = 값, 키 = 값, 키 = 값}
        // key 하나당 value는 하나입니다. 즉, 키가 같다면 value는 가장 최근 걸로 덮어쓰기됩니다.
        strMap1.put("kor2025005", "kimFive");   // 김오 → kimFive
         ...
         */
        System.out.println(strMap1.get("kor2025001") + "님");  // value값만 출력 : 김일
        ...
        koreanScores2.put("김이", 87.4);
        koreanScores2.put("김삼", 64.1);
        koreanScores2.put("김사", 4.8);
        koreanScores2.put("김오", 100.0);
        koreanScores2.put("김육", 43.7);

        // 특정 key의 value 수정
        // # 1. 아까 말한 것처럼 key에는 value가 하나 밖에 없기 떄문에 최근걸로 덮어쓰기 됩니다.
        System.out.println(koreanScores2);
        koreanScores2.put("김오", 8.7);
        System.out.println(koreanScores2);
        ...
        /*
            ...
            없는 key라면 key를 새로 생성해서 value를 초기화 하지만
            있는 key라면 value에 재 대입을 하게 됩니다.
        ...
        System.out.println("kor2025006 존재 여부 : " + searchKeyFlag1);     // false

        // 특정 value 존재 여부 → containsValue(); → boolean
        boolean searchValue1 = strMap1.containsValue("김삼");
        System.out.println("김삼 존재 여부 : " + searchValue1);       // true

        // Map의 엔트리로부터 Set을 생성
        Set<Map.Entry<String, String>> entrySet1 = new HashSet<>();
        // 이상의 코드는 Set을 하나 만드는데, 내부에 Key가 Stirng, Value가 String인 Map이 통째로
        // 들어간다는 것을 의미합니다.
        Set<Map.Entry<String, String>> entrySet2 = strMap1.entrySet();      // 중첩 generic
        // 이상의 코드는 strMap1의 key-value 쌍을 Set으로 바꾸는 메서드인 .entrySet()의 호출 결과를
        // entrySet2라는 변수에 담았음을 뜻합니다.
        System.out.println(strMap1);        // {키=값, 키=값, 키=값}
        System.out.println(entrySet2);      // [element, element, element]
        System.out.println();
