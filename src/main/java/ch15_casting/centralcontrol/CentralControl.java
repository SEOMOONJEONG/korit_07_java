...
//            System.out.println(device.getClass().getSimpleName() + "장치 연결 실패");
            return;       // 메서드 종료 키워드
        }
        ...
        /*
            ...
            현재 보시면 method의 결과값을 가지고 다시 . 찍어서 그 다음  method를 출력했습니다.
            이상의 개념을 chaining method라고 해서 return값의 유형을 명확하게 알고 있어야
            그 다음 어떤 method를 실행시킬 것인지를 알 수 있습니다.
         */
    }

    private int checkEmpty() {  // 메서드인데 private 썼습니다.
        // Main에서 굳이 몇 개나 더 전자제품을 추가할 수 있는지 알 필요가 없을것 같아서요.
        ...
        /*
            ...
            음수값을 지정하는 경우도 있는데,
            나중에 위에 있는 addDevice() 메서드에서 if(checkEmpty() == -89234)로 쓰고 싶지 않으므로
            -1을 쓰는게 가장 보편적입니다.
         */
    }
    ...
    /*
        ...
        deviceArray를 반복 돌려서 몇 번 인덱스에 어떤 클래스의 객체가 있는 지 표시하는
        showInfo 메서드를 정의하시오.
        실행 예
        슬롯 [ 1 ] 번 : Computer
        슬롯 [ 2 ] 번 : LED
    ...
    public void showInfo() {
        for(int i = 0; i < deviceArray.length; i++){
            ...
            // deviceArray → deviceArray[i] 배열
        }
    }

    // 이제 배열 내부를 각 element들의 고유 메서드들을 실행시켜볼겁니다.
    // 논리적으로는 말이 안됩니다...

    public void performSpecificMethod() {
        for(Power device : deviceArray) {
            if(device instanceof AirConditioner) { // Power의 서브클래스가 잘못된 다운캐스팅을 하지 않도록 하는 조건은
                ...
                printer.print();
                                ...
                                // 추후 update를 통해서 다른 기기들을 지원할 때는 } else 사이에 else if문을 추가해아만 함
    ...
    // print() 메소드도 centralControl을 통해 실행하시오
