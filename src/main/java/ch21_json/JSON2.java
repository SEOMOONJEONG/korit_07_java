...
public class JSON2 {
    public static void main(String[] args) {
        ...
        /*
            ...
            그리고 gson 객체에서도 toJson()이 있고 gsonBuilder 에서도 toJson()이 있따는 점 또한 주목할 부분입니다.
        ...
        /*
            jsonBuilder로 fromJson()을 적용하더라도 콘솔 상에서
            prettyPrinting은 적용되지 않습니다.
            결과값 :
            {"username":"kim1","password":"9876","email":"a@test.com","name":"김일","age":"20"}
            이상의 이유는 fromJson()이라는 메서드 명에서 알 수 있듯이
            JsonData(즉 String Data)를 Map으로 바꾸는 겁니다.
            String의 경우에는 개행을 통해서 prettyPrinting이 적용될 수 있지만,
            Map은 toString() 메서드를 재정의하지 않으면 default값 형태로 콘솔에 출력이 되겠죠,
            그래서 gson.fromJson()이든 gsonBuilder.fromJson()이건 콘솔에 찍히는
            결과값은 동일하다고 할 수 있습니다.
         */
        Map<String, String> userMap3 = gson.fromJson(jsonUser2, Map.class);
        Map<String, String> userMap4 = gsonBuilder.fromJson(jsonUser2, Map.class);
